...
M5_Clock の current release-driven development で使用する Operating Model を定義します。
...
- Arduino IDE / board package / M5Unified / device selection remain firmware toolchain authority; mise does not replace them.
- source compile success does not prove flashed-device behavior; bind host/device verification to board model, firmware artifact, and source SHA.
- `credentials.h` remains untracked local secret material per current project setup. Do not commit it or infer values. Migration to another secret provider requires a separate decision.
- NTP/network-dependent behavior is validated separately from offline UI/timer logic.

## Agent Skills lifecycle

- Bun exists only as auxiliary Skills CLI tooling.
